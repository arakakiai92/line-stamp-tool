...
st.caption("縦・横構図の自動判別＆任意レイアウト（カスタム分割）に対応した【超軽量・オンデマンド処理版】です。")
...
if st.session_state.get('video_path') is not None:
    ...
    # 判別結果のインフォメーション表示
    if is_vertical:
        st.info(f"📱 **縦構図動画** を検出しました（解像度: {w} × {h}）")
        default_options = [
            "6カット (2列 × 3行)",
            "8カット (2列 × 4行)",
            "4カット (2列 × 2行)",
        ...
        st.info(f"🖥️ **横構図動画** を検出しました（解像度: {w} × {h}）")
        default_options = [
            "10カット (5列 × 2行 = 計10個)",
            "12カット (4列 × 3行 = 計12個)",
            "6カット (2列 × 3行 = 計6個)",
...
if st.session_state.get('video_path') is not None:
    ...
    total_stamps = st.session_state.get('total_stamps', 10)
    ...
    with col_right:
        ...
        filter_noise = st.checkbox("🧹 見切れゴミ自動除去", value=True, key="filter_noise_cb")
        sharp_edge = st.checkbox("🔪 輪郭をぼかさずクッキリ保つ (シャープ透過)", value=True, key="sharp_edge_cb")
        
        st.markdown("**端のカット (見切れ・枠線削除)**")
